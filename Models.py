...
class Model_new_new (object):
    ...
    def model(self, x, args):
        ...
        Mod = Mod + args[3]*np.cos(x[1]) + args[4]*np.sin(x[1])
        Mod = Mod + args[5] * np.cos(2*x[1]) + args[6] * np.sin(2*x[1])
        # Компенсация маршевых двигателей
        Mod = Mod + args[7]*x[2]*np.abs(np.cos(x[0]))*np.sin(x[0]/2)
        Mod = Mod + args[8]*x[3]*np.sin(x[0]/2)
        # Компенсация работы батарей 50В
        Mod = Mod + args[9]*(x[4] - args[10])*np.sin(x[0]/2)
        # Компенсация угла дифферента
        Mod = Mod + args[11] * x[5] + args[12] * x[5]**2 + args[13] * x[5]**3
        return Mod
